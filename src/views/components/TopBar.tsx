...
const TopBar = () => {
  ...
  return (
    <Box display="flex" justifyContent="space-between" py={1} sx={{ px: { xs: 2, sm: 5 } }}>
      <Box display="flex">
        <Box
          alt={t("logo_LVP")}
          component="img"
        ...
        <Stack direction="row" sx={{ display: { xs: "none", md: "block" } }}>
          {menuItems.map((menuItem =>
             <Button key={menuItem} sx={{color: "text.secondary", "&:hover": { transform: "scale3d(1.1, 1.1, 1)" } }}>{t(menuItem)}</Button>
            ))}
        </Stack>
      </Box>
